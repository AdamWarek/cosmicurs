/**
 * Renders a spinning 3D Venus globe into #venus-3d canvas using Three.js.
 * Texture: images/3d/2k_venus_surface.jpg (equirectangular projection).
 */
import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.167.1/build/three.module.js';

const TEXTURE_PATH = 'images/3d/2k_venus_surface.jpg';
const ROTATION_SPEED = 0.003;

function initVenus3D() {
  const container = document.querySelector('.planet-venus');
  const canvas = document.getElementById('venus-3d');
  ...
  window.addEventListener('planet-grab-toggle', (e) => {
    if (e.detail.planet !== 'venus') return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initVenus3D);
} else {
  initVenus3D();
}
