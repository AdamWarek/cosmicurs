/**
 * Renders a spinning 3D Neptune globe into #neptune-3d canvas using Three.js.
 * Texture: images/3d/2k_neptune.jpg (equirectangular projection).
 */
import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.167.1/build/three.module.js';

const TEXTURE_PATH = 'images/3d/2k_neptune.jpg';
const ROTATION_SPEED = 0.003;

function initNeptune3D() {
  const container = document.querySelector('.planet-neptune');
  const canvas = document.getElementById('neptune-3d');
  ...
  window.addEventListener('planet-grab-toggle', (e) => {
    if (e.detail.planet !== 'neptune') return;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initNeptune3D);
} else {
  initNeptune3D();
}
